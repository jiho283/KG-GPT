    ...
    for divided_claim, corresponding_entity_set in divided_claim_list.items():
        if len(corresponding_entity_set) == 1 and len(corresponding_entity_set[0]) > 0:
            if len(list(KG[corresponding_entity_set[0]])) == 1:
                ...
                relation_retrieval_query = open_file('./meta_3hop_prompts/relation_retrieval_prompt.txt').replace('<<<<TOP_K>>>>', str(top_k)).replace('<<<<SENTENCE>>>>', divided_claim).replace('<<<<RELATION_SET>>>>', str(one_hop_relation_set))
        else:
            relation_retrieval_query = open_file('./meta_3hop_prompts/relation_retrieval_prompt.txt').replace('<<<<TOP_K>>>>', str(top_k)).replace('<<<<SENTENCE>>>>', divided_claim).replace('<<<<RELATION_SET>>>>', str(new_all_relation_set))
